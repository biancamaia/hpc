...
void potential(double **potential){
    ...
    for(i = 0; i < size; i++){
        for(j = 0; j < size; j++)
            potential[i][j] = 0.07*i*i + 0.01*i*i;
...
void finite_difference(double complex**p, double **im, double **r, double **potential, double alpha, double dt){
    ...
    #pragma omp parallel shared(p, im, r, potential, alpha, dt, chunk) private(i, j)
    {
        nthreads = omp_get_num_threads();
        chunk = size/nthreads;

        #pragma omp for schedule(dynamic, chunk) collapse(2) reduction(+:dt)
        for(i = 1; i < size - 1; i++){
            for(j = 1; j < size - 1; j++){
...
int main(){
    ...
    double pi = 2.0*atan(1.0), dx=0.1, x0 = 10.0, y0 = 10.0, sigma = 2.0;
    double k0x = 6.6*pi, k0y = 6.6*pi, dt=dx*dx/10.0, alpha = dt/2.0*dx*dx;
    int x, y;
    int chunk, nthreads, dti, tf=1000000;
    FILE *data;
    ...
    data = fopen("tempo_loop_200.txt", "w");

    #pragma omp parallel shared(psi, im_psi, r_psi, v, alpha) private(dti)
    {
        nthreads = omp_get_num_threads();
        chunk = tf/nthreads;
    ...
    for(x = 0; x < size; x++){
        for(y = 0; y < size; y++){
            fprintf(data, "%d   %d  %0.6g\n", x, y, fabs(((float) psi[x][y]))*fabs(((float) psi[x][y])));
        }
        fprintf(data, "\n");
    }
    fprintf(data, "\n");
